...
OUTPUT_DIR = Path(__file__).resolve().parent.parent / "results" / "submissions"
...
def main():
    print(" [V25 COMPONENT STACK] Processing Orders and AOV...")
    ...
    for comp, is_aov in components.items():
        ...
        print(f"   Recursive forecasting for {comp}...")
    ...
    print(f" Saved Raw: {out_path.name}")
    
    # Anchor with V18 a22 (The King)
    ref_path = V18_DIR / "submission_v18_dl_stack_anchor_a22.csv"
    if ref_path.exists():
        ...
        print(f" Saved Anchor 50%: {anchored_path.name}")
